...
def test_speech_to_text_success():
    with patch('services.eleven_labs_service.ElevenLabsBuisness.extract_audio_bytes_from_video', return_value=b'audio'):
...
def test_speech_to_text_file_not_found():
    with patch('services.eleven_labs_service.ElevenLabsBuisness.extract_audio_bytes_from_video', side_effect=FileNotFoundError('not found')):
